...
namespace ActiveDirectory_API.Repositories
{
    public class UserService : IUserService
    {
        ...
        public async Task<List<Designation>> GetDesignationDataFormatted()
        {
            try
            {
                using (DirectoryEntry entry = new DirectoryEntry())
                {
                    ...
                    if (results != null)
                    {
                        foreach (SearchResult result in results)
                        {
                            ...
                            designation.Name = result.Properties["title"].Count > 0 ? result.Properties["title"][0].ToString() : "N/A";
                    ...
                    return designations.GroupBy(d => d.Name).Select(group => group.First()).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<Department>> GetDepartmentDataFormatted()
        {
            try
            {
                using (DirectoryEntry entry = new DirectoryEntry())
                {
                    ...
                    if (results != null)
                    {
                        foreach (SearchResult result in results)
                        {
                            ...
                            department.Name = result.Properties["department"].Count > 0 ? result.Properties["department"][0].ToString() : "N/A";
                    ...
                    return departments.GroupBy(d => d.Name).Select(group => group.First()).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<Branch>> GetBranchDataFormatted()
        {
            try
            {
                using (DirectoryEntry entry = new DirectoryEntry())
                {
                    ...
                    if (results != null)
                    {
                        foreach (SearchResult result in results)
                        {
                            ...
                            branch.Name = result.Properties["physicaldeliveryofficename"].Count > 0 ? result.Properties["physicaldeliveryofficename"][0].ToString() : "N/A";
                    ...
                    return branches.GroupBy(b => b.Name).Select(group => group.First()).ToList();
                }
            }
            catch (Exception ex)
            {
